/**
 * OpenCode Plugin: vm-disk-cleanup
 * 
 * description: > VM / Session Disk Cleanup #Context 
 * Original: MSApps Claude Plugins (vm-disk-cleanup)
 */

export const VmDiskCleanupPlugin = async ({ project, client, $, directory, worktree }) => {
  console.log("vm-disk-cleanup plugin initialized!")

  return {
    // Tool definitions converted from SKILL.md
    tool: {
      // Add tools based on the original plugin functionality
      // Reference: /Users/michalshatz/Documents/Claude/claude-plugins/plugins/vm-disk-cleanup/skills/vm-cleanup/SKILL.md
    },

    // Event hooks
    event: async ({ event }) => {
      if (event.type === "session.created") {
        console.log("vm-disk-cleanup plugin ready")
      }
    },
    ...
    "session.created": async (input, output) => {
      await client.app.log({
        body: {
          service: "vm-disk-cleanup",
          level: "info",
          message: "vm-disk-cleanup plugin loaded",
          extra: { directory }
        }
      })
    }
